    ...
    return (
        <section className="page-swap page-from-right w-full max-w-2xl rounded-3xl border border-white/10 bg-white/5 p-8 text-center">
            ...
            <div className="mt-8 flex flex-col items-center gap-6">
                ...
                {outcomeText ? (
                    <div
                        className={`result-pop rounded-2xl border px-6 py-4 ${outcomeTone === "win"
                                ? "border-emerald-400/40 bg-emerald-400/10"
                                : "border-rose-400/40 bg-rose-400/10"
                            }`}
                    >
                        <div
                            className={`text-sm uppercase tracking-[0.35em] ${outcomeTone === "win" ? "text-emerald-200" : "text-rose-200"
                                }`}
                        >
                            {outcomeText}
                        </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-3">
                        {(["Heads", "Tails"] as CoinSide[]).map(side => (
                            <button
                                ...
                                disabled={isBusy}
                                className={`arcade-button rounded-2xl border px-4 py-3 text-sm font-semibold uppercase tracking-[0.2em] transition disabled:cursor-not-allowed disabled:opacity-50 ${choice === side
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-6">
                        {betOptions.map(amount => (
                            <button
                                ...
                                className={`arcade-button rounded-full border px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] transition disabled:cursor-not-allowed disabled:opacity-50 ${bet === amount
